...
# CLI
@click.group()
@click.version_option(version="1.0.0", prog_name="Content Generator")
def cli():
    """
    AI-Powered Content Generation CLI

    Generate reports, fiction, and presentations through natural language commands.
    """
    pass
...
def report(query, report_type, depth, max_results, output_format, html_template, html_theme, input_file, verbose):
    ...
    \b
        cli report ""
        cli report "" --type analysis --depth deep -o html
        cli report "" -t research -m 30 -o md -v
    """
...
async def _execute_report(query: str, report_type: str, depth: str, max_results: int,
    ...
    Args:
        query: Research query or topic for the report
        report_type: Type of report (business, academic, technical, consulting)
        depth: Search depth (overview, standard, comprehensive)
        max_results: Maximum number of search results to use
        output_format: Output format (md, html)
        html_template: HTML template name (if output_format is html)
        html_theme: HTML theme name (if output_format is html)
        input_file: Optional input file path for context
...
def fiction(query, genre, length, viewpoint, constraint, output_format, html_template, html_theme, input_file, verbose):
    ...
    \b
        cli fiction "" --genre mystery --length short
        cli fiction "" -g scifi -l medium -vp third -o html
        cli fiction "" -g mystery -c "" -o md -v
    """
...
async def _execute_fiction(query: str, genre: str, length: str, viewpoint: str,
                           ...
                           input_file: Optional[Path], verbose: bool):
    """
    Execute fiction generation command.
    
    Args:
        query: Story idea or prompt for the fiction
        genre: Fiction genre (mystery, scifi, fantasy, horror, romance, wuxia)
        length: Story length (short, medium, long)
        viewpoint: Narrative viewpoint (first, third, omniscient)
        constraints: List of additional constraints for the story
        output_format: Output format (md, html)
        html_template: HTML template name (if output_format is html)
        html_theme: HTML theme name (if output_format is html)
        input_file: Optional input file path for context
...
def ppt(topic, style, slides, depth, theme, speech_notes, input_file, verbose):
    ...
    \b
        cli ppt ""
        cli ppt "" --style business --slides 15
        cli ppt "2025" -s ted -n 8 -v
        cli ppt "" -s academic -d deep --theme blue
        cli ppt "" --speech-notes "" -v
    """
...
def export(project_id, export_type, output, verbose):
    ...
    \b
        cli export 20251004_215421_2025 --type pptx
        cli export 20251004_180344_ --type pdf -o report.pdf
        cli export 20251004_123456_ --type docx -v
    """
...
async def _execute_export(project_id: str, export_type: str, output_path: str, verbose: bool):
    """
    Execute export command to convert project to different formats.
    
    Args:
        project_id: Project ID (folder name in storage/)
        export_type: Export format (pdf, docx, pptx)
...
def iterate(project_id, requirement, verbose):
    ...
    \b
        cli iterate 20251004_215421 "3"
        cli iterate 20251004_180344 ""
        cli iterate 20251004_123456 "5" -v
    """
...
async def _execute_iterate(project_id: str, requirement: str, verbose: bool):
    """
    Execute iteration command to modify existing project.
    
    Args:
        project_id: Project ID (folder name in storage/)
        requirement: Modification requirement description
        verbose: Whether to show verbose output
    """
...
def ask(question, model, verbose):
    """
    Ask a question to the AI (placeholder for future implementation).

    Args:
        question: The question to ask
        model: Model to use (quality, speed, etc.)
        verbose: Whether to show verbose output

    Examples:
        cli ask "What is Python?"
        cli ask "Python" --model quality -v
    """
    click.echo(click.style("\n=== Ask ===\n", fg="blue", bold=True))
    click.echo(f"Question: {question}\n")

    # TODO: Implement ask functionality
    click.echo(click.style("  Feature not yet implemented...\n", fg="yellow"))
...
def _display_result(result: dict, verbose: bool, output_type: str = 'report', output_format: str = 'md'):
    """
    Display generation result to the user.
    
    Args:
        result: Result dictionary from agent
        verbose: Whether to show verbose output
        output_type: Type of output (report, fiction, ppt)
        output_format: Output format (md, html)
    """
