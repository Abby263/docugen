...
class ImageProcessor:
    ...
    def insert_images_to_content(
        ...
        if mode == "top":
            ...
            logger.warning(f"Unknown mode: {mode}, using bottom mode")
            return ImageProcessor._insert_images_at_bottom(content, images)
    ...
    def _generate_image_gallery(
        ...
        for i, img in enumerate(images, 1):
            ...
            metadata_parts = []
            if width and height:
                metadata_parts.append(f"Size: {width}x{height}")
            if photographer:
                if photographer_url:
                    metadata_parts.append(f"Photo by: [{photographer}]({photographer_url})")
                else:
                    metadata_parts.append(f"Photo by: {photographer}")
    ...
    def enhance_search_results_with_images(
        ...
        for result in results:
            ...
            if full_content and images:
                ...
                logger.info(
                    f"Inserted {len(images)} images into result: "
