...
class PPTCoordinator:
    ...
    def __init__(
        ...
        # Design coordinator - generates global design specification
    ...
    async def generate_ppt_v3(
        ...
        """
        Generate multi-page HTML PPT (V3 Architecture)

        Uses multi-page HTML architecture where each slide is an independent HTML file.
        Reuses V2's PageAgent to generate detailed content.
        ...
        Returns:
            {
                ...
                "ppt_dir": "PPT directory path",
                "total_slides": 10,
                "slide_files": [...],
                "index_page": "Navigation page path",
                "presenter_page": "Presenter mode page path"
            }
        """
        logger.info(f"[{self.name}] Generating multi-page HTML PPT (V3): {topic}")
        logger.info(f"[{self.name}] PPT config: {ppt_config}")
        ...
        try:
            ...
            logger.info(f"[{self.name}] Phase 1: Generating PPT outline (target: {slides_count} slides)")
            print(f"\n📋 Generating PPT outline... (target: {slides_count} slides)")
            outline = await self._generate_outline_v2(topic, search_results, style, slides_count)
            print(f"✅ Outline generation complete! Generated {len(outline['pages'])} slides")

            # Phase 1.5: Generate global design specification (NEW)
            logger.info(f"[{self.name}] Phase 1.5: Generating global design specification")
            print(f"\n🎨 Generating global design specification...")
            design_spec = await self.design_coordinator.generate_design_spec(
                topic=topic,
                outline=outline,
                style=style
            )
            logger.info(f"[{self.name}] Design spec: {design_spec.layout_style} style, primary color {design_spec.primary_color}")
            print(f"✅ Design specification complete! Style: {design_spec.layout_style}, Primary color: {design_spec.primary_color}")

            # Phase 2: Use PageAgent to generate detailed HTML content for each page (reuse V2 logic)
            total_pages = len(outline['pages'])
            logger.info(f"[{self.name}] Phase 2: Generating detailed content for each page ({total_pages} pages)")
            print(f"\n📄 Generating {total_pages} pages in parallel...")
            print(f"   Note: LLM is processing, this may take a few minutes...")
            page_results = await self._parallel_generate_pages(
                outline=outline,
                search_results=search_results,
                style=style,
                speech_scene=None,  # V3 doesn't need speech notes
                design_spec=design_spec  # Pass global design specification
            )
            success_count = sum(1 for r in page_results if r.get('html_content'))
            print(f"✅ Page content generation complete! Success: {success_count}/{total_pages} pages")

            # Phase 3: Convert page content to slide data structure
            logger.info(f"[{self.name}] Phase 3: Building slide data structure")
            print(f"\n🔧 Building slide data structure...")
            slides_data = self._convert_pages_to_slides_data(outline, page_results)
            print(f"✅ Data structure build complete!")

            # Phase 4: Use MultiSlidePPTGenerator to generate multi-page HTML PPT files
            logger.info(f"[{self.name}] Phase 4: Generating multi-page HTML files")
            print(f"\n📦 Generating multi-page HTML files and navigation pages...")
            ...
            logger.info(f"[{self.name}] Multi-page HTML PPT generation complete")
            print(f"✅ PPT generation complete!")
            print(f"\n🎉 Generation successful!")
            print(f"   📁 PPT directory: {result.get('ppt_dir')}")
            print(f"   📄 Total slides: {result.get('total_slides')}")
            print(f"   🏠 Index page: {result.get('index_page')}")
            print(f"   🎬 Presenter page: {result.get('presenter_page')}")
            return result

        except Exception as e:
            logger.error(f"[{self.name}] Multi-page HTML PPT generation failed: {e}")
            import traceback
            traceback.print_exc()
            return {
                "status": "error",
    ...
    def _convert_outline_to_slides_data(
        ...
        """
        Convert outline to slide data

        Args:
            outline: PPT outline
            search_results: Search results for content filling
        ...
        for i, page in enumerate(outline['pages']):
            ...
            if slide_type == 'cover':
                ...
                for idx, p in enumerate(content_pages[:6], 1):  # Maximum 6 sections
                    sections.append({
                        'number': idx,
                        'title': p.get('topic', ''),
                ...
                slide_data['content'] = {
                    ...
                    'points': [{'text': p, 'icon': 'check'} for p in points] if points else [
                        {'text': 'Thank you for watching', 'icon': 'heart'}
                    ],
                    'closing': 'Thank you!'
                }
    ...
    def _convert_pages_to_slides_data(
        ...
        """
        Convert PageAgent-generated page HTML to slide data

        Args:
            outline: PPT outline
            page_results: List of pages generated by PageAgent, each containing html_content
        ...
        for i, page in enumerate(page_results):
            ...
            # Get page type and title from outline
            ...
            slide_data = {
                ...
                'title': topic,
                'template': self._get_template_for_type(slide_type),
                # Store PageAgent-generated HTML content directly
                'html_content': html_content
            }
    ...
    def _get_template_for_type(self, slide_type: str) -> str:
        """Return template name based on slide type"""
        template_mapping = {
...
"""
    ...
    async def _parallel_generate_pages(
        ...
        design_spec: Optional[DesignSpec] = None  # New: global design specification
    ) -> List[Dict[str, Any]]:
        """
        Phase 2: Generate HTML content for each page

        Uses PageAgent to generate detailed HTML content for each page
        """
        from .page_agent import PageAgent, PageSpec, GlobalContext

        # Build global context - use design_spec if available, otherwise use outline colors
        colors_to_use = outline['colors']
        if design_spec:
            # Use design specification color scheme
            colors_to_use = {
        ...
        global_context = GlobalContext(
            ...
            speech_scene=speech_scene  # Speech scene context
        )

        # Summarize search results
        content_summary = self._summarize_search_results(search_results)

        # Build CSS guide - include design specification info if design_spec exists
        css_guide = self._get_css_component_guide()
        if design_spec:
            css_guide += f"""

# Global Design Specification (IMPORTANT - Must strictly follow!)
**Color Scheme:**
- Primary: {design_spec.primary_color}
- Secondary: {design_spec.secondary_color}
- Accent: {design_spec.accent_color}
- Background: {design_spec.background_color}
- Text: {design_spec.text_color}
- Secondary Text: {design_spec.text_secondary_color}

**Font Specification:**
- Font: {design_spec.font_family}
- Title Font Size: {design_spec.title_font_size}
- Content Font Size: {design_spec.content_font_size}

**Visual Style:**
- Layout Style: {design_spec.layout_style}
- Spacing: {design_spec.spacing}
- Border Radius: {design_spec.border_radius}
- Shadows: {'Enabled' if design_spec.use_shadows else 'Disabled'}
- Gradients: {'Enabled' if design_spec.use_gradients else 'Disabled'}
- Animation: {design_spec.animation_style}

**Chart Colors (for Chart.js):**
{', '.join(design_spec.chart_colors)}

**Important Note:**
All pages must use the unified design specification above! Do not change colors, fonts, or styles on your own!
"""
        ...
        # Use progress display for parallel generation
        total = len(tasks)
        print(f"   [0/{total}] Starting generation...")
        ...
        for i, result in enumerate(page_results):
            if isinstance(result, Exception):
                failed += 1
                logger.error(f"[{self.name}] Page {i+1} generation failed: {result}")
                print(f"   ❌ Page {i+1} generation failed: {str(result)[:50]}")
                # fallback
                results.append({
                    "slide_number": i + 1,
                    "html_content": f"<div class='flex items-center justify-center h-full'><p class='text-2xl'>Generation failed</p></div>",
                    "speech_notes": None
                })
            else:
                success += 1
                ...
                # Output progress after each page completion
                print(f"   ✓ [{success}/{total}] Page {i+1} generation complete")

        print(f"\n   📊 Generation statistics: {success} successful, {failed} failed")
        return results
