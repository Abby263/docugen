...
class ReportCoordinator:
    ...
    async def generate_report(
        ...
        try:
            ...
            if (
                ...
                logger.info(f"[{self.name}] Phase 3.6: Adding images...")
    ...
    async def _add_images_to_sections(
        ...
        """
        Add images to sections using ImageSearcher
        """
        enhanced_sections = []
        
        logger.info(f"[{self.name}] Searching for images for {len(sections)} sections...")
        
        for section in sections:
            # Skip if already has images
            if section.get("images_inserted"):
            ...
            try:
                title = section.get("title", "")
                query = f"{title} related high quality image"
                
                logger.info(f"[{self.name}] Searching images for: {title}")
                images = await self.image_searcher.search(query, limit=images_per_section)
                
                if images:
                    # Download images if project_id is provided
                    if project_id and self.image_downloader:
                        downloaded_images = await self.image_downloader.download_images(
                            images, 
                            project_id=project_id,
                            subfolder="report_images"
                        )
                        section["images"] = downloaded_images
                    else:
                        section["images"] = images
                        
                    section["images_inserted"] = True
                    logger.info(f"[{self.name}] Added {len(images)} images to section '{title}'")
                
            except Exception as e:
                logger.warning(f"[{self.name}] Failed to add images to section '{title}': {e}")
